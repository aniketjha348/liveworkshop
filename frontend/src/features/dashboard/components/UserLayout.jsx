 ...
 * Sidebar navigation layout for user dashboard with fixed header and sidebar
 */
...
const UserLayout = () => {
  ...
  const navItems = [
    { icon: LayoutDashboard, label: 'My Learning', path: ROUTES.DASHBOARD },
    { icon: Compass, label: 'Browse Workshops', path: ROUTES.HOME },
  ];
                     ...
                     {profileOpen && (
                        <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-100 py-2 animate-in fade-in slide-in-from-top-2 z-50">
                            ...
                            <div className="px-2 py-1">
                                <button onClick={() => {navigate(ROUTES.USER_PROFILE); setProfileOpen(false)}} className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-blue-600 w-full text-left rounded-lg transition-colors">
