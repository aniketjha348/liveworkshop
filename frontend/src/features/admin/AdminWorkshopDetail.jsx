              ...
              <div className="space-y-4">
                <div className="flex gap-4 items-end">
                  <div className="flex-1">
                    <label className="block text-sm font-medium text-slate-700 mb-1">Add Reminder (Hours Before)</label>
                    <Input 
                      type="number" 
                  ...
                  <Button onClick={addReminder} variant="outline" className="border-dashed border-2 border-slate-300 hover:border-violet-500 hover:text-violet-600">
                ...
                <div className="space-y-3 mt-4">
                  {reminders.length === 0 ? (
                    <p className="text-slate-400 italic text-center py-4 border-2 border-dashed border-slate-100 rounded-lg">
                      No specific reminders set. Default (24h) will apply if empty.
                    </p>
                  ) : (
                    reminders.map((reminder, index) => (
                      <div key={index} className="flex items-center justify-between p-4 bg-white border border-slate-200 rounded-lg shadow-sm group hover:border-violet-200 transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-slate-800">{reminder.hours_before} Hours Before Start</p>
                            <p className="text-xs text-slate-500">Subject: {reminder.subject || 'Default Reminder'}</p>
                          </div>
                        </div>
                        <Button 
                          size="sm" 
